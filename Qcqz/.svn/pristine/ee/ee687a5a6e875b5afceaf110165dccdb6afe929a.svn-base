...
//		initUserRole();
	...
	private void initMenu() {
		...
		wdgl.setTauth(null);
		authDao.saveOrUpdate(wdgl);

		Tauth sxdy = new Tauth();
		sxdy.setCid("sxdy");
		sxdy.setCname("属性定义");
		...
		authDao.saveOrUpdate(sxdy);

		Tauth lbdy = new Tauth();
		lbdy.setCid("lbdy");
		lbdy.setCname("类别定义");
		...
		lbdy.setTauth(wdgl);
		authDao.saveOrUpdate(lbdy);
		
		Tauth sjlr = new Tauth();
		sjlr.setCid("sjlr");
		sjlr.setCname("数据录入");
		...
		sjlr.setTauth(wdgl);
		authDao.saveOrUpdate(sjlr);
		
		Tauth zdgl = new Tauth();
		zdgl.setCid("zdgl");
		zdgl.setCname("字典管理");
		zdgl.setCseq(BigDecimal.valueOf(4));
		zdgl.setCurl("dataEnter/letterData.jsp");
		zdgl.setCiconcls("icon-group");
		zdgl.setTauth(wdgl);
		authDao.saveOrUpdate(zdgl);
		
		Tauth bjgl = new Tauth();
		bjgl.setCid("bjgl");
		bjgl.setCname("班级管理");
		...
		bjgl.setTauth(wdgl);
		authDao.saveOrUpdate(bjgl);
		Tauth xsgl = new Tauth();
		xsgl.setCid("xsgl");
		xsgl.setCname("学生管理");
		...
		xsgl.setTauth(wdgl);
		authDao.saveOrUpdate(xsgl);
		
		Tauth kclb = new Tauth();
		kclb.setCid("wdgl");
		kclb.setCname("课程列表");
		...
		kclb.setTauth(wdgl);
		authDao.saveOrUpdate(kclb);
		
		Tauth jwgl = new Tauth();
		jwgl.setCid("jwgl");
		jwgl.setCname("教务管理");
		...
		cp.setTauth(jwgl);
		authDao.saveOrUpdate(cp);
		
		Tauth skjl = new Tauth();
		skjl.setCid("skjl");
		skjl.setCname("授课管理");
		skjl.setCseq(BigDecimal.valueOf(3));
		skjl.setCurl("");
		skjl.setCiconcls("icon-orange");
		skjl.setTauth(null);
		authDao.saveOrUpdate(skjl);
		
		Tauth sk = new Tauth();
		sk.setCid("sk");
		sk.setCname("授课");
		sk.setCseq(BigDecimal.valueOf(1));
		sk.setCurl("shouke/shouke.jsp");
		sk.setCiconcls("icon-role");
		sk.setTauth(skjl);
		authDao.saveOrUpdate(sk);
	}
