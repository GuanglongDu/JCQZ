...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			method:'get',
			url : '<%=basePath%>categories/getAll',
			pagination : true,
			pagePosition : 'bottom',
			pageSize : 10,
	...
	function edit() {
		...
		if (rows.length == 1) {
			var p = dj.dialog({
				title : '编辑用户',
				href : '<%=basePath%>categories/editCategories',
				width : 450,
				height : 200,
				buttons : [ {
					text : '编辑',
	...
	function append() {
		var p = dj.dialog({
			title : '添加用户',
			href : '<%=basePath%>categories/addCategories',
			width : 440,
			height : 200,
			buttons : [ {
				text : '添加',
		...
		if (rows.length > 0) {
			dj.messagerConfirm('请确认', '您要删除当前所选项目？', function(r) {
				if (r) {
					...
					$.ajax({
						url : '<%=basePath%>categories/deleteCategories',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
