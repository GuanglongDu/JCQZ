			...
			var p = dj.dialog({
				...
				onLoad : function() {
					...
					var ptree = pid.combotree({
						lines : true,
						url : '<%=basePath%>menu/doNotNeedSession_treeRecursive',
	...
	function append() {
		var p = dj.dialog({
			...
			onLoad : function() {
				...
				var ptree = pid.combotree({
					lines : true,
					url : '<%=basePath%>menu/doNotNeedSession_tree'
				});
			}
		});
	}
